...
@section('title', 'Unggah Materi Baru')
@section('page_title', 'Unggah Materi Baru')
...
<form action="{{ route('teacher.materials.store') }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="card" style="max-width: 800px;">
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <small style="color: var(--on-surface-variant); font-size: 12px; margin-top: 0.25rem; display: block;">
                    Hanya menampilkan kelas yang terdaftar dalam jadwal Anda.
                </small>
            </div>
        ...
        <div class="form-group">
            ...
            <input type="text" name="title" id="title" class="form-control" value="{{ old('title') }}" placeholder="Masukkan judul materi pembelajaran..." required>
        </div>

        <div class="form-group">
            <label for="content" class="form-label">Konten Materi <span style="color: red;">*</span></label>
            <textarea name="content" id="content" class="form-control" rows="8" placeholder="Tulis konten atau materi pembelajaran secara detail di sini. Anda juga bisa menyisipkan tautan eksternal (Google Drive/YouTube)..." required style="resize: vertical; font-family: inherit;">{{ old('content') }}</textarea>
        </div>
        ...
        <div class="form-group">
            ...
            @include('components.file-uploader', ['inputName' => 'attachments[]', 'uploaderId' => 'mat-create'])
        </div>
