    ...
    <div class="card" style="max-width: 800px;">
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="text" name="title" id="title" class="form-control" value="{{ old('title', $assignment->title) }}" placeholder="Masukkan judul..." required>
            </div>

            <div class="form-group">
                <label for="due_date" class="form-label">Tenggat Waktu Pengumpulan <span style="color: red;">*</span></label>
                <input type="datetime-local" name="due_date" id="due_date" class="form-control" value="{{ old('due_date', date('Y-m-d\TH:i', strtotime($assignment->due_date))) }}" required>
            </div>
        </div>

        <div class="form-group">
            <label for="description" class="form-label">Deskripsi / Detail Instruksi Tugas <span style="color: red;">*</span></label>
            <textarea name="description" id="description" class="form-control" rows="8" placeholder="Masukkan detail tugas..." required style="resize: vertical; font-family: inherit;">{{ old('description', $assignment->description) }}</textarea>
        </div>

        <div class="form-group">
            <label class="form-label">Lampiran File Soal <small style="color: var(--on-surface-variant); font-weight: 400;">(Opsional — maks. 20 MB/file)</small></label>
            @include('components.attachment-current', ['attachment' => $assignment->attachment, 'inputId' => 'asgn-edit'])
            @include('components.file-uploader', ['inputName' => 'attachments[]', 'uploaderId' => 'asgn-edit-new'])
        </div>
