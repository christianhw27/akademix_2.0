    ...
    <div class="card">
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="text" name="username" id="username" class="form-control" value="{{ old('username', $guardian->user->username) }}" placeholder="Contoh: ortu.andi">
            </div>
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="email" name="email" id="email" class="form-control" value="{{ old('email', $guardian->user->email) }}" placeholder="ortu@sekolah.sch.id">
            </div>

            <div class="form-group">
                <label for="is_active" class="form-label">Status Akun <span style="color: red;">*</span></label>
                <select name="is_active" id="is_active" class="form-select" required>
                    <option value="1" {{ old('is_active', $guardian->user->is_active) == '1' ? 'selected' : '' }}>Aktif</option>
                    <option value="0" {{ old('is_active', $guardian->user->is_active) == '0' ? 'selected' : '' }}>Nonaktif</option>
                </select>
            </div>
        </div>
    </div>
    ...
    <div class="card">
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="text" name="full_name" id="full_name" class="form-control" value="{{ old('full_name', $guardian->user->full_name) }}" required>
            </div>

            <div class="form-group">
                <label for="phone" class="form-label">Nomor Telepon / WhatsApp</label>
                <input type="text" name="phone" id="phone" class="form-control" value="{{ old('phone', $guardian->phone) }}" placeholder="Contoh: 081211111111">
            </div>
        </div>

        <div class="form-group">
            <label for="address" class="form-label">Alamat Rumah</label>
            <textarea name="address" id="address" class="form-control" rows="3" placeholder="Alamat rumah tinggal...">{{ old('address', $guardian->address) }}</textarea>
        </div>
    </div>
